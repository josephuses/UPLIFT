...
\Bold{Dividing by Zero} Everyone knows that $\dfrac{0}{2}=0$, the problem is that far too many students say that
$\frac{2}{0}=0$ or $\frac{2}{0}=2$. Remember that division by zero is undefined.
\Bold{Bad/Lost/Assumed Parenthesis} There are a lot of errors that students commonly make here. The first error is that
...
\end{center}
\end{example}
\Bold{Improper Distribution} Be careful when using distribution property, two errors that are common to
...
\end{example}
\Bold{The Mad Slasher} The Mad Slasher is when, in the haste of the moment, start crossing out similar
...
\end{example}
\Bold{Linear Dysfunction Disorder (LDD)}
...
\end{equation*}
\end{example}
\Bold{Multiplying two numbers using difference of two squares}
...
\end{equation*}
\end{example}
\Bold{Divide by the factors of the divisors one at a time}
